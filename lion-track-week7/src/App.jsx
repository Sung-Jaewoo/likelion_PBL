...
function getViewOptionsFromLocation() {
  const params = new URLSearchParams(window.location.search);

  return {
    filter: params.get("part") || defaultViewOptions.filter,
    sort: params.get("sort") || defaultViewOptions.sort,
    search: params.get("search") || defaultViewOptions.search,
  };
}

function buildListUrl({ filter, sort, search }) {
...
function App() {
  ...
  const [viewOptions, setViewOptions] = useState(getViewOptionsFromLocation);
  ...
  const moveToDetail = (lionId) => {
    window.history.pushState(null, "", `/lions/${encodeURIComponent(lionId)}`);
    setRoute({ page: "detail", lionId: String(lionId) });
  };

  const moveToList = () => {
    const url = buildListUrl(viewOptions);
      ...
      <section className="control-panel">
        ...
        <div className="control-row filter-row">
          ...
          <select
            id="filterSelect"
            value={filter}
            onChange={(event) => updateViewOption("filter", event.target.value)}
          >
          ...
          <select
            id="sortSelect"
            value={sort}
            onChange={(event) => updateViewOption("sort", event.target.value)}
          >
