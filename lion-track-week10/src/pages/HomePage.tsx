...
function HomePage({
  ...
  const showLoginRequired = () => {
    setNotice("명단을 수정하려면 로그인이 필요합니다.");
  };
  ...
  const handleDeleteLast = async () => {
    ...
    if (lions.length === 0) {
      setNotice("삭제할 아기 사자가 없습니다.");
      return;
    }

    const success = await onDelete(lions[0].id);
    if (success) setNotice("가장 최근 아기 사자를 삭제했습니다.");
  };
  ...
  const handleRandomCreate = async (count: number) => {
    ...
    const success = await onCreateRandom(count, user.id);
    if (success) setNotice(`랜덤 아기 사자 ${count}명을 추가했습니다.`);
  };
  ...
  return (
    <main className="app-shell">
      ...
      <section className="control-panel" aria-label="명단 관리 도구">
          ...
          <form onSubmit={handleSubmit}>
            <div className="form-grid">
              ...
              <div>
                ...
                <input id="grade" value={form.grade} onChange={handleFormChange} placeholder="3" />
              </div>
