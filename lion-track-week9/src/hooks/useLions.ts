...
export function useLions() {
  ...
  const createRandomLions = async (count: number, ownerId: string) => {
    ...
    try {
      ...
      const rows = randomData.results.map((user) => mapLionFormToInsert(mapRandomUserToForm(user), ownerId));
      ...
      setErrorMessage(error instanceof Error ? error.message : "랜덤 추가 중 오류가 발생했습니다.");
      setStatus("오류 발생");
      setIsSaving(false);
      return false;
    }
